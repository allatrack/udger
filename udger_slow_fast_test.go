...
func TestParseUaSF(t *testing.T) {
	log.Println("Start TestParseUaSF")
	...
	data, _ := udgerFS.ParseUa(userAgent)
	...
	// test crawler
	data, _ = udgerFS.ParseUa(`192.comAgent`)

	assert.Equal("Crawler", data.Class)
	assert.Equal("crawler", data.ClassCode)
}

func TestParseIpSF(t *testing.T) {
	log.Println("Start TestParseIpSF")

	assert := assert.New(t)
	ipAddress := udgerFS.ParseIp(`1.0.128.191`)
	...
	ipAddress = udgerFS.ParseIp(`2001:e42:101::`)
